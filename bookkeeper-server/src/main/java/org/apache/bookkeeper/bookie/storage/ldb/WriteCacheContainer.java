...
import static org.apache.bookkeeper.bookie.storage.ldb.WriteCacheManager.commonWriteCacheQueue;
...
public class WriteCacheContainer {
    ...
    public WriteCacheContainer(EntryLogger entryLogger, LedgerMetadataIndex ledgerIndex, EntryLocationIndex entryLocationIndex,
                               DbLedgerStorageStats dbLedgerStorageStats, ScheduledExecutorService cleanupExecutor, Counter flushExecutorTime,
                               long writeCacheSize, long perDirectoryBlockWriteCacheSize, ByteBufAllocator allocator) {
        ...
        for (int i = 0; i < writeCacheSize / perDirectoryBlockWriteCacheSize; i++) {
            WriteCache writeCache = new WriteCache(allocator, perDirectoryBlockWriteCacheSize);
    ...
    public boolean checkpoint(WriteCache writeCacheBeingFlushed) throws IOException {
        ...
        try {
            ...
            if (LOG.isDebugEnabled()) {
                LOG.debug("DB batch flushed time : {} s", MathUtils.elapsedNanos(batchFlushStartTime) / (double) TimeUnit.SECONDS.toNanos(1));
            }
            ...
            if (writeCacheBeingFlushed.isSingleDirectoryWriteCache()) {
                freeBlockCaches.add(writeCacheBeingFlushed);
            } else {
                commonWriteCacheQueue.add(writeCacheBeingFlushed);
            }
    ...
    private class FlushWriteCacheThread extends BookieCriticalThread {
        ...
        @Override
        public void run() {
            while (running || !fulledBlockCaches.isEmpty()) {
